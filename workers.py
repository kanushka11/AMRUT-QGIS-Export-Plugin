...
class LayerValidationWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            QgsMessageLog.logMessage('Validation started.', 'MyPlugin', Qgis.Info)  # Check if the task is started
            valid = geometry.check_geometries_and_extents(self.layers)
            QgsMessageLog.logMessage('Validation Completed.', 'MyPlugin', Qgis.Info)  # Check result
...
class GridLayerValidationWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            QgsMessageLog.logMessage('Validation error : ' + str(e), 'MyPlugin', Qgis.Critical)
            self.result_signal.emit(False, str(e))
            self.finished.emit() # Emit error message
